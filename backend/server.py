...
if getattr(sys, 'frozen', False):
    ...
    sys.path.append(str((BASE_DIR / "ml-sharp" / "src").absolute()))

from sharp.models import PredictorParams, create_predictor
from sharp.models.params import InitializerParams
from sharp.utils import io
from sharp.utils.gaussians import unproject_gaussians, Gaussians3D
...
app = FastAPI(title="OpticSplat API")
...
def init_model(stride=2):
    print(f"[SERVER] Initializing model on device: {DEVICE} (stride={stride})", flush=True)
    ...
    try:
        ...
        predictor.eval()
        predictor.to(DEVICE)
        print("[SERVER] Model initialized successfully.", flush=True)
        return predictor
    except Exception as e:
        print(f"[SERVER] Failed to initialize model: {e}", flush=True)
        raise
...
def process_single_frame(predictor, image_np, internal_shape, decimate=1):
    ...
    with torch.no_grad():
        ...
        if decimate > 1:
            ...
            print(f"[SERVER] Decimated Gaussians to {gaussians.mean_vectors.shape[1]} splats", flush=True)
            
    return gaussians

@app.post("/generate")
async def generate_3dgs(file: UploadFile = File(...), name: str = Form(None)):
    print(f"[SERVER] Received generation request for file: {file.filename}, custom name: {name}", flush=True)
    ...
    print(f"[SERVER] Saving input to: {input_path}", flush=True)
    ...
    def perform_generation():
        try:
            ...
            fps = 0
            if is_video:
                print(f"[SERVER] Processing video file...", flush=True)
                import imageio
            ...
            if not is_video:
                try:
                    ...
                    print(f"[SERVER] Metadata extraction warning: {e}", flush=True)
            ...
            print(f"[SERVER] Initial Metadata: Res={width}x{height}, Aspect={aspect_ratio:.2f}, Focal35={f_35mm}mm", flush=True)

            # Load AI Model
            stride = 2
            print(f"[SERVER] Loading AI Model to {DEVICE} with stride {stride}...", flush=True)
            ...
            if is_video:
                ...
                print(f"[SERVER] Video length: {num_frames} frames. Processing up to {max_frames} (step={step})...", flush=True)
                ...
                for i, frame in enumerate(reader):
                    ...
                    print(f"[SERVER] Processing frame {i} (Index {frame_idx_in_sequence})...", flush=True)
            ...
            metadata = {
                ...
                "input_url": f"/output/{file_id}/input{input_ext}",
            ...
            print(f"[SERVER] CRITICAL ERROR during generation: {str(e)}", flush=True)
            import traceback
            traceback.print_exc()
            raise e
    ...
    try:
        ...
        result = await anyio.to_thread.run_sync(perform_generation)
        return result
    except Exception as e:
...
if __name__ == "__main__":
    import uvicorn
    print("\n[SERVER] API Backend started at http://127.0.0.1:8000")
